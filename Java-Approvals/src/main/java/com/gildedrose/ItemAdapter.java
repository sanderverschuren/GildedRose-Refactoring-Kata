package com.gildedrose;

public class ItemAdapter {
    public static final int MAX_QUALITY = 50;
    public static final int MIN_QUALITY = 0;
    public static final String BACKSTAGE_PASS = "Backstage passes to a TAFKAL80ETC concert";
    public static final String AGED_BRIE = "Aged Brie";
    public static final String SULFURAS = "Sulfuras, Hand of Ragnaros";
    public static final String CONJURED = "Conjured Mana Cake";
    private final Item item;
    private final String name;
    private final int quality;
    private final int sellIn;

    public ItemAdapter(Item item) {
        this.item = item;
        this.name = item.name;
        this.sellIn = item.sellIn;
        this.quality = item.quality;
    }

    public Item getItem(){
        return item;
    }

    public ItemAdapter progressToNextDay(){
        return updateSellIn().updateQuality();
    }

    private ItemAdapter updateSellIn() {
        int decrement = switch (name) {
            case SULFURAS -> 0;
            default -> 1;
        };
        return new ItemAdapter(new Item(name, sellIn - decrement, quality));
    }

    private ItemAdapter updateQuality() {
        int qualityIncrement = switch (name) {
            case SULFURAS -> 0;
            case BACKSTAGE_PASS -> {
                if (isExpired()) yield -quality;
                else if (isExpiredInDays(5)) yield 3;
                else if (isExpiredInDays(10)) yield 2;
                yield 1;
            }
            case AGED_BRIE -> {
                if (isExpired()) yield 2;
                yield 1;
            }
            case CONJURED -> {
                if (isExpired()) yield -4;
                yield -2;
            }
            default -> {
                if (isExpired()) yield -2;
                yield -1;
            }
        };
        return new ItemAdapter(new Item(name, sellIn, calculateQuality(qualityIncrement)));
    }

    private boolean isExpiredInDays(int days) {
        return sellIn < days;
    }

    private boolean isExpired() {
        return isExpiredInDays(0);
    }

    private int calculateQuality(int increment) {
        if (increment == 0)
            return quality;
        return limitQualityRange(quality + increment);
    }

    private static int limitQualityRange(int value) {
        return Math.min(Math.max(value, MIN_QUALITY), MAX_QUALITY);
    }
}
